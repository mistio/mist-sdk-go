...
// All allowed values of ClusterProviders enum
var AllowedClusterProvidersEnumValues = []ClusterProviders{
	"amazon",
	"azure",
	"digitalocean",
	"google",
...
func (v *ClusterProviders) UnmarshalJSON(src []byte) error {
	...
	for _, existing := range AllowedClusterProvidersEnumValues {
...
// NewClusterProvidersFromValue returns a pointer to a valid ClusterProviders
// for the value passed as argument, or an error if the value passed is not allowed by the enum
func NewClusterProvidersFromValue(v string) (*ClusterProviders, error) {
	ev := ClusterProviders(v)
	if ev.IsValid() {
		return &ev, nil
	} else {
		return nil, fmt.Errorf("invalid value '%v' for ClusterProviders: valid values are %v", v, AllowedClusterProvidersEnumValues)
	}
}

// IsValid return true if the value is valid for the enum, false otherwise
func (v ClusterProviders) IsValid() bool {
	for _, existing := range AllowedClusterProvidersEnumValues {
		if existing == v {
			return true
		}
